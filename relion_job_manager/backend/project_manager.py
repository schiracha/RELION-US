"""
project_manager.py — lets the app point at any RELION project directory at
runtime instead of a folder hardcoded relative to where the backend happens
to live ("Do I have to run this from the working directory?"). Also owns
lightweight persistence of run-history *summaries* per project, so switching
projects and reloading the GUI shows that project's own job history rather
than whatever the app's in-memory state happens to contain (which is reset
...
it has a `.relion_job_manager/` marker directory created by *this* app the
first time it was pointed at that folder. Anything else is treated as "not
a project yet" and the frontend prompts: start a new project here, or pick
a different folder.
...
MARKER_DIRNAME = ".relion_job_manager"
